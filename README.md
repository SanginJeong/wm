...
_오픈 API 연동 기반 · Web (PWA) 전용 · 백엔드 없음_
...
> 개발 환경에서는 **MSW**가 모든 API를 모킹합니다. 실제 키 없이도 바로 개발 가능합니다.
...
|            | 기술              | 비고                          |
| ---------- | ----------------- | ----------------------------- |
| 컴포넌트   | **shadcn/ui**     | Radix UI 기반, 직접 소유 방식 |
| 스타일     | **Tailwind CSS**  | v4, shadcn 기본 시스템        |
...
| API 모킹 | **MSW**   | Service Worker 기반, 개발 환경 전용 |
...
| 포매터   | **Prettier**            |                           |
| Git Hook | **Husky + lint-staged** | 커밋 전 자동 린팅         |
...
> **MSW 자동 활성화**: `NODE_ENV=development` 환경에서는 Service Worker가 자동으로 실행되어  
> 실제 API 키 없이도 모든 기능을 로컬에서 사용할 수 있습니다.
...
# ── 개발 환경 (자동 설정됨, 수동 변경 가능) ──────────
# VITE_MSW_ENABLED=true                  # MSW 강제 활성화/비활성화
```
...
style: ProductCard 컴포넌트 Tailwind 정리
...
**WONDERMALL** · v2.0 · 2026.03.06

_Web (PWA) Only · No Backend · Open API Integration_
